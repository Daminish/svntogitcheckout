...
CREATE TABLE `capco_travel_portal`.`cab_details` (
  ...
  `business_purpose` VARCHAR(45) NOT NULL,
...
INSERT INTO `capco_travel_portal`.`perdiem_details` (`country`, `perdiem`, `currency`) VALUES ('India', '10000', 'INR');
INSERT INTO `capco_travel_portal`.`perdiem_details` (`country`, `perdiem`, `currency`) VALUES ('USA', '5000', 'USD');
INSERT INTO `capco_travel_portal`.`perdiem_details` (`country`, `perdiem`, `currency`) VALUES ('Euro', '3000', 'EUR');
...
INSERT INTO `capco_travel_portal`.`employee_details` (`name`, `email_id`, `employee_id`,`capco_user_id`,`grade`, `mobile_number`, `is_approver`, `created_on`, `modified_on`, `employee_location`, `project_code`,`token_id`) VALUES ('Pranjal', 'shubham.kale@capco.com', '16290','PNRT','M0', '99992222', '0', '2017-07-07', '2017-07-07', 'pune', '8888',5678);
INSERT INTO `capco_travel_portal`.`employee_details` (`name`, `email_id`, `employee_id`,`capco_user_id`,`grade`, `mobile_number`, `is_approver`, `created_on`, `modified_on`, `employee_location`, `project_code`,`token_id`) VALUES ('Rajnish', 'Rajnish@capco.com', '16292','RDUB','M7', '99992222', '1', '2017-07-07', '2017-07-07', 'Banglore', '9999',9876);
...
from cab_details c inner join main_request m on m.request_id=c.request_id where m.requested_by=id and m.current_status not in('approved','rejected','Cancelled'));

CREATE FUNCTION getCabApproved(id int)
RETURNS int DETERMINISTIC
RETURN (select count(c.request_id)  
from cab_details c inner join main_request m on m.request_id=c.request_id where m.requested_by=id and m.current_status ='approved');


CREATE FUNCTION getAccomodationApproved(id int)
RETURNS int DETERMINISTIC
RETURN (select count(c.request_id) 
from accomodation_details c inner join main_request m on m.request_id=c.request_id where m.requested_by=id and m.current_status ='approved');

CREATE FUNCTION getAccomodationPending(id int)
RETURNS int DETERMINISTIC
RETURN (select count(c.request_id) 
from accomodation_details c inner join main_request m on m.request_id=c.request_id where m.requested_by=id and m.current_status not in('approved','rejected','Cancelled'));

CREATE FUNCTION getFlightPending(id int)
RETURNS int DETERMINISTIC
RETURN (select count(c.request_id) 
from flight_details c inner join main_request m on m.request_id=c.request_id where m.requested_by=id and m.current_status  not in('approved','rejected','Cancelled'));

CREATE FUNCTION getFlightApproved(id int)
RETURNS int DETERMINISTIC
RETURN (select count(c.request_id) 
from flight_details c inner join main_request m on m.request_id=c.request_id where m.requested_by=id and m.current_status ='approved');

CREATE FUNCTION getForexApproved(id int)
RETURNS int DETERMINISTIC
RETURN (select count(c.request_id) 
from forex_details c inner join main_request m on m.request_id=c.request_id where m.requested_by=id and m.current_status ='approved');

CREATE FUNCTION getForexPending(id int)
RETURNS int DETERMINISTIC
RETURN (select count(c.request_id) 
from forex_details c inner join main_request m on m.request_id=c.request_id where m.requested_by=id and m.current_status  not in('approved','rejected','Cancelled'));

CREATE FUNCTION getVisaApproved(id int)
RETURNS int DETERMINISTIC
RETURN (select count(c.request_id) 
from visa_request c inner join main_request m on m.request_id=c.request_id where m.requested_by=id and m.current_status ='approved');

CREATE FUNCTION getVisaPending(id int)
RETURNS int DETERMINISTIC
RETURN (select count(c.request_id) 
from visa_request c inner join main_request m on m.request_id=c.request_id where m.requested_by=id and m.current_status  not in('approved','rejected','Cancelled'));
