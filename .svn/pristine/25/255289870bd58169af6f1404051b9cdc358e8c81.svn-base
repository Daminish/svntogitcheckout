...
public class TravelRequestController {
	...
	public ResponseEntity<Void> addTravelRequest(@RequestBody TravelRequestDTO travelRequest,
		...
		if((tokenId != null && tokenId != 0) && mainRequestService.tokenValidation(tokenId, userId)) {
			...
			if(newRequest.getCabRequest()==null && newRequest.getAccomodationRequest()==null && newRequest.getFlightRequest()==null && newRequest.getForexRequest()==null && newRequest.getVisaRequestVO()==null) {
			...
			if (mainRequestService.validateTravelRequestObject(newRequest) != null) {
				...
				try {
					...
					logger.info("TravelRequestController : addTravelRequest: new Travel Request Id :" + travelRequestId);
	 ...
	 */
	@RequestMapping(value = "/request/{requestId}", method = RequestMethod.POST, headers = "Accept=application/json", produces = "application/json")
	public ResponseEntity<TravelRequestDTO> updateTravelRequest(@RequestBody TravelRequestDTO travelRequest,
			@PathVariable("requestId") String reqId, HttpServletRequest request) throws NumberFormatException, TravelServiceException {
		...
		if((tokenId != null && tokenId != 0) && mainRequestService.tokenValidation(tokenId, userId)) {
			...
			try {
				...
				if (updateRequest == null || mainRequestBO == null) {
					logger.info("travel request with requestId " + requestId + " not found");
					return new ResponseEntity("Data with Requested Id not found.",HttpStatus.NOT_FOUND);
				} else {
				...
				logger.info("TravelRequestController : updateTravelRequest: ended");
			} catch (TravelServiceException e) {
	...
	public ResponseEntity<Void> cancelTravelRequest(@PathVariable("requestId") Integer requestId, HttpServletRequest request) throws TravelServiceException {
		...
		if((tokenId != null && tokenId != 0) && mainRequestService.tokenValidation(tokenId, userId)) {
			...
			try {
				...
				if (mainRequestBO.getCurrentStatus().equals(TravelConstants.L1_PENDING)||mainRequestBO.getCurrentStatus().equals(TravelConstants.PENDING)) {
	...
	public ResponseEntity<TravelRequestDTO> getTravelRequest(@PathVariable("requestId") String reqId, HttpServletRequest request) throws NumberFormatException, TravelServiceException {
	 ...
	 * @throws TravelServiceException 
	 *//*
	@RequestMapping(value = "/getAllRequestHistory", method = RequestMethod.GET, headers = "Accept=application/json")
	public ResponseEntity<List<RequestHistoryLogVO>> getAllRequestHistoryLogs(HttpServletRequest request) throws TravelServiceException {
		...
		if((tokenId != null && tokenId != 0) && mainRequestService.tokenValidation(tokenId, userId)) {
			...
			try {
				listVO = mainRequestService.getAllRequestHistoryLogs();
				logger.info("TravelRequestController : getAllRequestHistoryLogs : Ended");
			} catch (TravelServiceException ex) {
				logger.error("TravelRequestController : getAllRequestHistoryLogs : TravelServiceException Caught : " + ex);
			}
	  ...
	  *//**
	  * @author e5544354 
	  *  This method to get all entries from request_history table by request_id
	  * @methodName getRequestHistoryByRequestId
	  * @param int
	  * @return List<RequestHistoryLogVO>
	  * @throws TravelServiceException 
	  *//*
	@RequestMapping(value = "/getRequestHistoryByRequestId/{requestId}", method = RequestMethod.GET, headers = "Accept=application/json")
	public ResponseEntity<List<RequestHistoryLogVO>> getRequestHistoryLogsByRequestId(@PathVariable("requestId") int requestId, HttpServletRequest request) throws TravelServiceException {
		logger.info("TravelRequestController : getRequestHistoryLogsByRequestId : Started");
		HttpSession session = request.getSession();		
		Integer tokenId = (Integer) session.getAttribute("tokenId");
		String userId = (String) session.getAttribute("userId");
		System.out.println(tokenId+" "+userId);
		if((tokenId != null && tokenId != 0) && mainRequestService.tokenValidation(tokenId, userId)) {
			List<RequestHistoryLogVO> listVO = null;
			try {
				listVO = mainRequestService.getRequestHistoryLogsByRequestId(requestId);
				logger.info("TravelRequestController : getRequestHistoryLogsByRequestId : Ended");
			} catch (TravelServiceException ex) {
				logger.error("TravelRequestController : getRequestHistoryLogsByRequestId : TravelServiceException Caught : "
						+ ex);
			}
			return new ResponseEntity(listVO, HttpStatus.OK);
		} else {
	   ...
	   *//**
	   * @author e5544354 
	   * This method to get all entries from request_history table by updated_by
	   * @methodName getRequestHistoryLogsByUpdatedBy
	   * @param int
	   * @return List<RequestHistoryLogVO>
	   * @throws TravelServiceException 
	   *//*
	@RequestMapping(value = "/getRequestHistoryByUpdatedBy/{updatedBy}", method = RequestMethod.GET, headers = "Accept=application/json")
	public ResponseEntity<List<RequestHistoryLogVO>> getRequestHistoryLogsByUpdatedBy(@PathVariable("updatedBy") int updatedBy, HttpServletRequest request) throws TravelServiceException {
		logger.info("TravelRequestController : getRequestHistoryLogsByUpdatedBy : Started");
		HttpSession session = request.getSession();		
		Integer tokenId = (Integer) session.getAttribute("tokenId");
		String userId = (String) session.getAttribute("userId");
		System.out.println(tokenId+" "+userId);
		if((tokenId != null && tokenId != 0) && mainRequestService.tokenValidation(tokenId, userId)) {
			List<RequestHistoryLogVO> listVO = null;
			try {
				listVO = mainRequestService.getRequestHistoryLogsByUpdatedBy(updatedBy);
				logger.info("TravelRequestController : getRequestHistoryLogsByUpdatedBy : Ended");
			} catch (TravelServiceException ex) {
				logger.error("TravelRequestController : getRequestHistoryLogsByUpdatedBy : TravelServiceException Caught : "
						+ ex);
			}
			return new ResponseEntity(listVO, HttpStatus.OK);
		} else {
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	@RequestMapping(value = "/getLatestApprovedRequest/{requestedBy}", method = RequestMethod.GET, headers = "Accept=application/json")
	public ResponseEntity<RequestsDTO> getLatestApprovedRequest(@PathVariable("requestedBy") int requestedBy,
			HttpServletRequest request) throws TravelServiceException {
		...
		if ((tokenId != null && tokenId != 0) && mainRequestService.tokenValidation(tokenId, userId)) {
			...
			if (requestedBy == 0) {
				...
				try {
					listVO = mainRequestService.getLatestApprovedRequest(requestedBy);
					if (listVO != null) {
						requestsDTO = new RequestsDTO();
						requestsDTO.setMainRequestVOList(listVO);
						RequestCountVO requestCountVO = requestCountService.getRequestCount(requestedBy);
			...
			logger.info("TravelRequestController : getLatestApprovedRequest : token not found for requested user");
			return new ResponseEntity("token not found for requested user", HttpStatus.BAD_REQUEST);
		}
	}
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	@RequestMapping(value = "/getRequestDetailsByUserId/{requestedBy}", method = RequestMethod.GET, headers = "Accept=application/json")
	public ResponseEntity<List<MainRequestVO>> getRequestDetailsByUserId(@PathVariable("requestedBy") int requestedBy,
			HttpServletRequest request) throws TravelServiceException {
		...
		if ((tokenId != null && tokenId != 0) && mainRequestService.tokenValidation(tokenId, userId)) {
			...
			if (requestedBy == 0) {
				...
				try {
					listVO = mainRequestService.getRequestDetailsByRequestedId(requestedBy);
